Feature list



* Settings menu

  * sound
  * Exponent notaion view / Natural Language Naming view (toggle)
  * Aspect ratios
  * more as needed

* upgrade menu (mostly finished)

  * change all goblin parameters into global variables within the controller object
  * make the goblin coin tracker more stable and easier to reference in other menus (done)
  * make a temp upgrade menu including

    * goblin types
    * goblin lifetime
    * max goblins
    * goblin spawn speed
    * goblin value
    * goblin death value

  * track collected goblins

    * show only collected goblins in gob book
    * shade out unseen goblins

* Goblin throne to set a fav goblin

  * make fav goblin the desktop pet
  * make fav goblin invincible

* Transparent window (will need to learn how to use DLL using C#)

  * gobbies jump on task bar
  * make the goblins half size
  * change throwing physics so they dont land on grab\_y

* Virtual Pet

  * Happyness

    * Minigames to boost morale

  * Hunger

    * Treatts

  * Dirtiness

    * Mud bath

* (eva specific task) Re-order goblin sprite list to have better progression
* make minigames that are unlocked by spotting specific goblins
